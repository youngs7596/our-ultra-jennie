...
1. [개요](#1-개요)
2. [장중 가격 시뮬레이션](#2-장중-가격-시뮬레이션)
3. [파일 구조](#3-파일-구조)
4. [단일 백테스트 실행](#4-단일-백테스트-실행)
5. [자동 파라미터 최적화](#5-자동-파라미터-최적화)
6. [결과 분석](#6-결과-분석)
7. [최적 결과 적용](#7-최적-결과-적용)
8. [전략 프리셋](#8-전략-프리셋)
9. [v1.0 개선사항](#9-v10-개선사항)
10. [트러블슈팅](#10-트러블슈팅)
...
실제 트레이딩은 장중(09:00~15:30)에 발생하지만, 보유한 데이터는 **일봉(OHLC)** 뿐입니다. 
틱 데이터나 분봉 데이터 없이도 장중 매매를 시뮬레이션하기 위해, 일봉의 시가/고가/저가/종가를 활용하여 
**가상의 장중 가격 경로**를 생성합니다.

### 시뮬레이션 패턴: V자 형태

현재 백테스트는 **V자 패턴**을 사용합니다:
...
**가격 경로:** `Open → Low → High → Close`

### 구현 상세

```python
def _simulate_intraday_path(self, open_price, high_price, low_price, close_price, slots):
    """
    V자 형태의 결정론적 장중 가격 경로 생성
    ...
    Returns:
        장중 가격 리스트 [slot_0_price, slot_1_price, ..., slot_19_price]
    """
    # 주요 포인트: Open → Low → High → Close
...
| 설정 | 값 | 설명 |
|------|-----|------|
| 장 시작 | 09:00 | `MARKET_OPEN_MINUTES = 540` |
| 장 마감 | 15:20 | `MARKET_CLOSE_MINUTES = 920` |
| 슬롯 간격 | 20분 | `INTRADAY_INTERVAL_MINUTES = 20` |
| 총 슬롯 수 | 20개 | `(920-540)/20 + 1 = 20` |

### 슬롯별 가격 분배 예시

**예: Open=100원, Low=95원, High=110원, Close=105원**

| 슬롯 | 시간 | 구간 | 대략적 가격 |
|------|------|------|------------|
| 0 | 09:00 | 시작 | 100원 (Open) |
| 1~5 | 09:20~10:40 | Open → Low | 100 → 95원 |
| 6~12 | 11:00~13:00 | Low → High | 95 → 110원 |
| 13~19 | 13:20~15:20 | High → Close | 110 → 105원 |

### 코사인 이징 (Cosine Easing)

급격한 가격 변화가 아닌 **자연스러운 곡선**으로 전환합니다:
...
| 한계점 | 설명 | 영향 |
|--------|------|------|
| **고정 패턴** | 항상 V자 형태 사용 | 실제는 A자, N자, W자 등 다양 |
| **저가 먼저 가정** | Low가 High보다 먼저 발생 | 일부 날은 반대일 수 있음 |
| **결정론적** | 같은 일봉 → 같은 경로 | 실제 장중 변동성 미반영 |
| **갭 미반영** | 전일 종가와 당일 시가 갭 무시 | 시초가 급등락 미반영 |

### 왜 V자 패턴인가?

1. **보수적 시뮬레이션**: 저가를 먼저 경험하므로 손절 조건이 일찍 트리거됨
2. **재현 가능성**: 랜덤 요소 없이 동일 결과 보장
3. **최악의 경우 대비**: 장 초반 급락 후 회복하는 패턴 가정

### 다른 패턴으로 변경하려면?

`_simulate_intraday_path()` 함수의 `key_points` 순서를 변경:

```python
# A자 패턴 (시가 → 고가 → 저가 → 종가)
key_points = [open_price, high_price, low_price, close_price]

# N자 패턴 (시가 → 고가 → 저가 → 고가 → 종가)
key_points = [open_price, high_price, low_price, high_price * 0.95, close_price]
```
