...
def get_redis_connection():
    """
    Redis 연결 객체를 반환합니다 (Singleton).
    Cloud Run 환경에서는 환경변수 REDIS_HOST, REDIS_PORT를 사용합니다.
    """
...
def set_market_regime_cache(regime_payload: dict, ttl_seconds: int = 3600):
    """
    ...
    try:
        redis_client.setex(MARKET_REGIME_CACHE_KEY, ttl_seconds, json.dumps(payload))
...
def get_market_regime_cache(max_age_seconds: int = 3600):
    """
    ...
    try:
        data_json = redis_client.get(MARKET_REGIME_CACHE_KEY)
        if not data_json:
            return None
...
def set_sentiment_score(stock_code: str, score: int, reason: str):
    """
...
def get_sentiment_score(stock_code: str) -> dict:
    """
    [Redis] 종목의 실시간 감성 점수를 조회합니다.
    Returns: {'score': 50, 'reason': 'No Data'} (기본값)
    """
    r = get_redis_connection()
