...
CREATE TABLE BACKTEST_TRADELOG (
  LOG_ID            NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY,
  TRADE_DATE        DATE NOT NULL,
  STOCK_CODE        VARCHAR2(16) NOT NULL,
  STOCK_NAME        VARCHAR2(128),
  TRADE_TYPE        VARCHAR2(8) NOT NULL, -- BUY / SELL
  QUANTITY          NUMBER,
  PRICE             NUMBER,
  REASON            VARCHAR2(500),
  STRATEGY_SIGNAL   VARCHAR2(64),
  KEY_METRICS_JSON  CLOB,
  REGIME            VARCHAR2(32),
  CREATED_AT        TIMESTAMP DEFAULT SYSTIMESTAMP,
  CONSTRAINT PK_BACKTEST_TRADELOG PRIMARY KEY (LOG_ID)
)
"""
...
def ensure_backtest_log_table(connection):
    ...
    try:
        ...
        cur.execute("""
            SELECT COUNT(*) 
            FROM user_tables 
            WHERE table_name = 'BACKTEST_TRADELOG'
        """)
...
def main():
    ...
    try:
        ...
        # [추가] ConfigManager가 사용할 DB 연결 풀 초기화
        try:
            database.init_connection_pool(
                ...
                db_service_name=os.getenv("OCI_DB_SERVICE_NAME"),
                wallet_path=wallet_path,
                min_sessions=1, # 백테스트는 단일 스레드이므로 최소 연결
                max_sessions=2
            )
            logger.info("✅ DB 연결 풀 초기화 완료 (for ConfigManager)")
        except Exception as e:
            logger.error(f"❌ DB 연결 풀 초기화 실패: {e}")
            # 풀 초기화에 실패해도 단일 연결로 계속 진행
